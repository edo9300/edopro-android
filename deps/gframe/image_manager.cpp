...
const fschar_t* GetExtension(int type) {
	if(type == PNG_FILE)
		return TEXT(".png");
	else if(type == JPG_FILE)
		return TEXT(".jpg");
	return TEXT("");
}
#if 0
void ImageManager::DownloadPic(int code) {
	...
	path_string name = fmt::format(TEXT("{}/./pics/temp/{}"),mainGame->working_directory.c_str(), code);
	path_string ext;
	pic_download.lock();
...
// 					fp.close();
					int res = porting::downloadFile(fmt::format(src.url, code).c_str(), name.c_str());
					if(res != 0) {
